...
class MappingPipelinePathHandler(PipelinePathHandler):
	...
	def __init__(self, workflow, test_config=False, **kwargs):
		...
		if test_config and not set(self.sample_ids).issubset(set(self.wildcard_values["sample"])):
			raise ValueError(
				"Error in config file: not all specified samples could be found using given input path pattern")
	...
	def _test_config_input(self, test_allowed_wildcards):
		...
		if not set(self.out_path_wildcards) == set(self.log_path_wildcards) == set(self.in_path_wildcards) | set(
			...
			raise ValueError("Error in config file: out_path_pattern, log_path_pattern and in_path_pattern "
				...
				set(self.out_path_wildcards), set(self.log_path_wildcards), set(self.in_path_wildcards)))
	...
	def _write_log(self, **kwargs):
		...
		with open(filename, "w") as f:
			f.write("pattern input:\n   in path pattern: {}\n   out path pattern: {}\n   log path pattern: {}\n\n"
				...
				self.samples, self.wildcard_values, self.wildcard_combs,
	...
	def log_generated_files(self, save_to="", path_pattern=None, **kwargs):
		...
		if not save_to:
			save_to = self.file_path(step="MappingPipelinePathHandler", extension="tsv", batch="all_batches",
									 flowcell="all_flowcells", lane="all_lanes", library="all_libraries",
...
class CovariateFileTool(PipelinePathHandler):
	...
	def update_covariate_data(self, step, extension):
		...
		self.covariate_data = pd.DataFrame(
			{"filename": files, "md5": md5, "group": group, "replicate": replicate, "label": label})
...
class SampleInfoTool(PipelinePathHandler):
	...
	def _get_wildcard_values_from_read_input(self, unix_style=True):
		...
		wildcard_values = {w: [] for w in wildcards}
		for inp in input_files:
			self._get_wildcard_values_from_file_path(inp, self.in_path_pattern, wildc_val=wildcard_values,
													 unix_style=unix_style)

		return {
			**wildcard_values,
			"read_extension": [f.replace(re.match(match_pattern, f).group(0), "") for f in input_files if
							   re.match(match_pattern, f)]
			}
	...
	def update_sample_info(self, library_default="unstranded", add=False):
		...
		wildcard_combs = [comb for comb in self._get_wildcard_combinations(wildcard_values) if
						  comb.read_extension in self.allowed_read_extensions]
		print("\nextracted combinations:\n{}".format(
			"\n".join("\t".join(i) for i in [wildcard_combs[0]._fields] + wildcard_combs)))
...
class ReportTool(PipelinePathHandler):
	...
	def _insert_entry_name(self, text, name, results_path):
		entry_name = name[0]
		return text.replace(self.entry_name_wildcard[0], entry_name).replace(self.entry_name_wildcard[1],
																			 self._make_id(name, results_path))
	...
	def _assemble_entries(self, entries, path, snippet_name, entry_heading_code, results_key):
		...
		for entry in entries:
			...
			entry_text += [self._get_entry_heading(entry_heading_code, entry_name), "\n\n",
						   self._assemble_template(sub_snippet_list, path, snippet_name, entry_heading_code,
												   (entry_name, snippet_name), results_key)]
	...
	def _assemble_template(self, snippet_list, path, snippet_name, entry_heading_code, entry=("", ""),
		...
		if type(snippet_list) is str: snippet_list = self.report_snippet_defaults[
			snippet_name] if snippet_list == "__defaults__" else [snippet_list]
		for snippet in snippet_list:
			...
			if type(snippet) is str:
				...
				for i, results_path in (tup for tup in enumerate(self.use_results[results_key[1]]) if
					...
					if all(Path(self.path_handler.file_path(
							**dict(zip(self.req_fields, req.split("-")), path_pattern=results_path))).exists() for req
						   in requirements):
				...
				# --- open sub-template folder
					...
					for i, results_path in (tup for tup in enumerate(self.use_results[results_key[1]]) if
						...
						sub_section_text = self._assemble_template(snippet_value, path / snippet_key, snippet_key,
																   entry_heading_code, entry,
																   results_key=(i, results_key[1]))
						...
						if all(Path(self.path_handler.file_path(
								**dict(zip(self.req_fields, req.split("-")), path_pattern=results_path))).exists() for
							   req in requirements):
