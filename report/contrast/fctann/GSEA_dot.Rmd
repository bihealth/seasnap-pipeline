...
#### Generic gene-set enrichment plot {.tabset .tabset-pills}

```{r fctann_gseadot_{{ENTRY_ID}}, results="asis"}
...
for (gsea in GSEA) {
    ...
    for (theCategory in names(gsea$results)) {
        ...
        if (is.null(cp_res_df) || nrow(cp_res_df)<1) {
            ...
            try({print(dotplot(gsea$results[[theCategory]], showCategory=20)); cat("\n")})
        }
    }
}
```
