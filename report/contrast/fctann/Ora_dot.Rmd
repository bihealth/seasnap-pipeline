...
#### Generic over-representation plot {.tabset .tabset-pills}

```{r fctann_oradot_{{ENTRY_ID}}, results="asis"}
...
for (ora in ORA) {
    ...
    for (theCategory in names(ora$results)) {
        ...
        if (is.null(cp_res_df) || nrow(cp_res_df)<1) {
            ...
            try({print(dotplot(ora$results[[theCategory]], showCategory=20)); cat("\n")})
        }
    }
}
```
