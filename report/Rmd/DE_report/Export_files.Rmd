...
readme_f          <- file.path(export_dir, "README.txt")
```
...
export_ft <- data.frame(
  Description=c(
    ...
    "Raw counts, log CPM and rlog counts",
    "Results of differential expression analysis for all contrasts"),
  File=sprintf("%s", basename(.f)),
  stringsAsFactors=FALSE)
export_ft_2 <- tmod_ft %>%
  mutate(Description=sprintf("Results of gene enrichment analysis with tmod for contrast %s", contrast),
         File=sprintf("%s", basename(filename))) %>%
  dplyr::select(Description, File)

export_ft <- rbind(export_ft, export_ft_2) %>% select(File, Description)
 
# paste the rows of export_ft together
export_ft <- paste(export_ft$File, export_ft$Description, sep=": ") %>% paste(collapse="\n")


writeLines(export_ft, readme_f)
```
