...
## first, the PDF plot.
pdf_plot_f <- function(df, x, y, intgroups) {
  require(ggplot2)
  shape <- color <- NULL
  ...
  if("Color" %in% names(df)) {
	...
	ggplot(df, ggplot2::aes_string(x=x, y=y, color=color, shape=shape)) +
		geom_point() +
		labs(color=intgroups[1], shape=intgroups[2]) +
		coord_fixed()
}
...
for (i in 1:numplots) {
  print(pdf_plot_f(df, paste("PC", 2*i-1, sep=""), paste("PC", 2*i, sep=""), intgroups))
}
dev.off()
```

```{r normalisationQCPCA_plot_plotly, fig.height=7, fig.width=7}
library(plotly)
