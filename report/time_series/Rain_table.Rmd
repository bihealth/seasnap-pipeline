...
x <- config$time_series$time_points
x <- rep(x, times=sapply(config$time_series$samples, length))

counts_file <- data.frame(read.csv(subset(file_tab, step=="import_gene_counts" & extension=="tsv" & contrast=="all")$filename, sep="\t"))
ts_columns <- unlist(config$time_series$circadian_ts_cortex$samples)
